...
class Food{
  ...
  document.getElementById("start").addEventListener("click",()=>{
    getAllFood().then(e=>{
      let foods = e.map((e)=>{
        return new Food(e.category, e.category_id, e.image, e.discount, e.price, e.name, e.stock )
      })
    ...
    ;
    div.appendChild(p);
    document.getElementById(`${food.name}+`).addEventListener("click",()=>{food.addProduct();
    document.getElementById(`${food.name}id`).innerText=`Stock: ${food.stock}`})
    document.getElementById(`${food.name}-`).addEventListener("click",()=>{food.removeProduct();
    document.getElementById(`${food.name}id`).innerText=`Stock: ${food.stock}`})
      })
    })
  })
